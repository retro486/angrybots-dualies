...
function Awake () {		
	...
	var mJoysticks = Input.GetJoystickNames();
	Debug.Log("Detected joysticks: " + mJoysticks.Length);
	if(mJoysticks.Length > 0) {
		for(var i = 0; i < mJoysticks.Length; i++) {
			Debug.Log("Joystick detected: " + mJoysticks[i]);
		}
	}
...
function Start () {
	#if UNITY_IPHONE || UNITY_WP8 || UNITY_BLACKBERRY || UNITY_TIZEN
...
function Update () {
	...
	var cameraAdjustmentVector : Vector3 = Vector3.zero;
		...
		#if (UNITY_XBOX360 || UNITY_PS3 || UNITY_IPHONE || UNITY_ANDROID || UNITY_STANDALONE || UNITY_EDITOR)
			// On consoles use the analog sticks
			var axisX : float;
			var axisY : float;

			// Force support for dual-joystick - use right joystick for player "aim"
